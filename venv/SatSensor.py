    ...
    def _create_content(self, json_string):
        ...
        if(sat_json['info']['satcount'] > 0):
            ...
            d = {'k': unixstamp,
                 ...
                 'sum': 'Here are some of the sattelites currently passing over grossmont right now!',
                 'story': 'As of {} the following satelites are overhead: {}'.format(timestamp.strftime("%B %d, %Y at %I:%M:%S %p"), ', '.join(map(str,sats))),
                 'url img': self.get_featured_image()
                 }
            return [d]
        else:
            print("no sats")
